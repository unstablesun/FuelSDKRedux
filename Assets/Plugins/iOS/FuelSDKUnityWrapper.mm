...
#define NORMALIZED_JSON_DATATYPE_STRING	5
    ...
    BOOL iOSSyncVirtualGoods()
    {
        return [[fuel instance] syncVirtualGoods];
    }
    
    BOOL iOSAcknowledgeVirtualGoods(const char* transactionId, const char* acknowledgementTokensJSONCString, BOOL consumed)
    {
        ...
        NSArray *acknowledgementTokens = nil;

        if (acknowledgementTokensJSONCString) {
            NSError *error = nil;
            NSString* acknowledgementTokensJSONString = [NSString stringWithFormat:@"%s" , acknowledgementTokensJSONCString];
            NSData* acknowledgementTokensData = [acknowledgementTokensJSONString dataUsingEncoding:NSUTF8StringEncoding];

            acknowledgementTokens = [NSJSONSerialization JSONObjectWithData:acknowledgementTokensData
                                                                    options:NSJSONReadingMutableContainers
                                                                      error:&error];

            if ((error != nil) || (acknowledgementTokens == nil)) {
                return NO;
            }
        }

        return [[fuel instance] acknowledgeVirtualGoods:transactionIdString acknowledgementTokens:acknowledgementTokens consumed:consumed];
    }
    ...
    BOOL iOSSetLocalNotificationMessage(const char* localNotificationType, const char* message)
    {
        if (localNotificationType == nil) {
            return NO;
        }

        NSString* localNotificationTypeString = [NSString stringWithUTF8String:localNotificationType];
        fuellocalnotificationtype* type = [fuellocalnotificationtype findByAlias:[localNotificationTypeString lowercaseString]];
        ...
        return [fuel setLocalNotificationMessage:type
                                         message:messageString];
    }

    BOOL iOSSetLocalNotificationActive(const char* localNotificationType, BOOL active)
    {
        if (localNotificationType == nil) {
            return NO;
        }
        ...
        return [fuel setLocalNotificationActive:type active:active];
    }

    BOOL iOSIsLocalNotificationActive(const char* localNotificationType)
    {
        if (localNotificationType == nil) {
            return NO;
        }
    ...
    void iOSCancelAllNotifications()
    {
        [fuel cancelAllNotifications];
    }
    ...
    BOOL iOSIgniteGetSampleEvent(const char* tags)
    {
        NSArray* tagsArray= nil;
        ...
        fuelignite *gSDK = [fuelignite instance];
        return   [gSDK getSampleEvents:tagsArray];
    }
    ...
    BOOL iOSIgniteGetOffer(const char* offerID)
    {
        NSString* offerIDString = [NSString stringWithFormat:@"%s" , offerID];

        fuelignite *gSDK = [fuelignite instance];
        return [gSDK getOffer:offerIDString];
    }

    BOOL iOSIgniteAcceptOffer(const char* offerID)
    {
        NSString* offerIDString = [NSString stringWithFormat:@"%s" , offerID];

        fuelignite *gSDK = [fuelignite instance];
        return [gSDK acceptOffer:offerIDString];
    }
    
    BOOL iOSGetLocalizationFile()
    {
    ...
    //user methods
    BOOL iOSrequestUpdateUserInfo(const char* userDetails)
    {

        NSString* userDetailsString = [NSString stringWithUTF8String:userDetails];
        NSData* data = [userDetailsString dataUsingEncoding:NSUTF8StringEncoding];

        NSError *error = nil;
        NSDictionary *userDetailsDict = [NSJSONSerialization JSONObjectWithData:data
                                                                        options:NSJSONReadingMutableContainers
                                                                          error:&error];
        ...
        userDetailsDict = (NSDictionary*)normalizeJSONDictionary(userDetailsDict);
        ...
        return [[fuel instance] requestUpdateUserInfo:userDetailsDict];
    }
    
    BOOL iOSrequestUserAvatars()
    {
        return  [[fuel instance] requestUserAvatars];
    }

    BOOL iOSrequestUserInfo()
    {
